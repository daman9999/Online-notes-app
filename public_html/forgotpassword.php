...
if(empty($_POST["forgotpasswordmail"])){
    $errors .= $missingEmail;
}else{
    $email = filter_var($_POST["forgotpasswordmail"], FILTER_SANITIZE_STRING);
}
if($errors){
...
if( $count != 1 ){
    echo '<div class="alert alert-danger">That email does not exists</div>';  exit;
}
...
$message .= "http://localhost/online%20notes%20app/public_html/resetpassword.php?user_id=$user_id&key=$key";
if(mail($email, 'Reset your password', $message, 'From:'.'example@gmail.com')){
